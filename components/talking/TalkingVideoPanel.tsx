...
// Collapsible numbered section — lets the form open calm (one section at a time)
// while still letting you jump to any group in any order.
function Group({
  n,
  title,
  hint,
...
export default function TalkingVideoPanel() {
  ...
  // Completion flags — drive the "done" check shown on each collapsible section.
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5 sm:p-6 space-y-5">
      ...
      {/* 2. Product */}
      <Group n={2} title="What you're advertising" done={hasProduct}>
        <input
          type="text"
          value={name}
      ...
      <Group
        n={3}
        title="What they say"
        hint="The exact spoken words. Draft with AI then edit, or type your own. Leave blank to auto-write from the details above."
        done={!!script.trim()}
      >
      ...
      <Group
        n={4}
        title="Language, voice & format"
        hint="Language dubs the whole ad into that tongue. Voice picks the speaker; 480p is faster/cheaper, 720p is sharper."
      >
