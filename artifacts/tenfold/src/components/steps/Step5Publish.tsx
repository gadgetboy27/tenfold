...
export default function Step5Publish() {
  const { generatedAssets, selectedAnchorId, expansions } = useAppStore();
      ...
      >
        ...
        {Array.from({ length: 20 }).map((_, i) => (
          <motion.div
            ...
            className="absolute w-2 h-2 rounded-sm bg-primary z-0"
            style={{ 
              left: 0, top: 0,
              backgroundColor: Math.random() > 0.5 ? 'var(--color-primary)' : 'white'
            }}
          />
        ))}
        ...
        <div className="bg-card border border-border rounded-xl p-4 flex flex-col flex-1 overflow-y-auto space-y-6">
          ...
          <section>
            ...
            <div className="flex justify-between items-center mt-2">
              ...
              <span className={`text-xs ${caption.length > 2200 ? 'text-error' : 'text-muted-foreground'}`}>
                {caption.length} / 2200
              </span>
            </div>
          </section>
