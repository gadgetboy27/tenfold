...
export async function runContentPipeline(context: PipelineRunnerContext): Promise<void> {
  const { submissionId, workspaceId, userId, transcript, profileKey, db } = context;
  ...
  try {
    ...
    try {
      ...
      await updateStageStatus(db, submissionId, 'analyse', 'completed', analysisOutput);
    } catch (error) {
    ...
    const [repurposeResult, thumbnailsResult] = await Promise.allSettled([
      (async () => {
        ...
        await updateStageStatus(db, submissionId, 'repurpose', 'completed', output);
        return output;
      })(),
      (async () => {
        await updateStageStatus(db, submissionId, 'thumbnails', 'running');
        const output = await generateThumbnailConcepts(analysisOutput, submissionId);
        await updateStageStatus(db, submissionId, 'thumbnails', 'completed', output);
        return output;
      })(),
    ]);
    ...
    if (repurposeResult.status === 'fulfilled') {
      ...
      const msg = repurposeResult.reason instanceof Error ? repurposeResult.reason.message : 'Unknown error';
      await updateStageStatus(db, submissionId, 'repurpose', 'failed', null, msg);
      throw new Error(`Repurpose stage failed: ${msg}`);
    }

    if (thumbnailsResult.status === 'rejected') {
      const msg = thumbnailsResult.reason instanceof Error ? thumbnailsResult.reason.message : 'Unknown error';
      await updateStageStatus(db, submissionId, 'thumbnails', 'failed', null, msg);
    ...
    await updateStageStatus(db, submissionId, 'schedule', 'completed', scheduleOutput);

    await updateStageStatus(db, submissionId, 'publish', 'completed', { pending: true });
