...
import { Calendar, Send, Globe } from 'lucide-react';
import toast from 'react-hot-toast';
import { api } from '@/lib/api';

const PLATFORMS = [
  { id: 'linkedin',  name: 'LinkedIn',    icon: Globe, color: 'hover:text-[#0A66C2] hover:border-[#0A66C2]' },
  { id: 'twitter',   name: 'Twitter / X', icon: Globe, color: 'hover:text-white hover:border-white' },
  { id: 'instagram', name: 'Instagram',   icon: Globe, color: 'hover:text-[#E1306C] hover:border-[#E1306C]' },
  { id: 'facebook',  name: 'Facebook',    icon: Globe, color: 'hover:text-[#1877F2] hover:border-[#1877F2]' },
  { id: 'youtube',   name: 'YouTube',     icon: Globe, color: 'hover:text-[#FF0000] hover:border-[#FF0000]' },
];
...
export default function Step5Publish() {
  ...
  const toggle = (id: string) => setSelectedPlatforms(prev => prev.includes(id) ? prev.filter(p => p !== id) : [...prev, id]);

  const handlePublish = async () => {
    if (selectedPlatforms.length === 0) { toast.error('Select at least one platform'); return; }
    setIsPublishing(true);
    try {
      if (!currentCompositionId) throw new Error('No composition saved — complete Step 4 first');
      const res = await api('/api/publish', {
        method: 'POST',
        body: JSON.stringify({ compositionId: currentCompositionId, platforms: selectedPlatforms, caption, hashtags: [] }),
        workspaceSlug,
      });
      if (!res.ok) { const e = await res.json().catch(() => ({})) as { error?: string }; throw new Error(e.error ?? `Publish failed (${res.status})`); }
      setPublished(true);
    } catch (err: unknown) {
      ...
      >
        {Array.from({ length: 20 }).map((_, i) => (
          <motion.div
            ...
            style={{ left: 0, top: 0, backgroundColor: Math.random() > 0.5 ? '#7C5CFC' : 'white' }}
          />
        ))}
        ...
        <p className="text-muted-foreground text-lg mb-8 z-10 text-center max-w-md">
          Published to {selectedPlatforms.map(id => PLATFORMS.find(p => p.id === id)?.name).join(', ')}.
        </p>
        <div className="flex gap-4 z-10">
          <Button variant="outline" className="opacity-50 cursor-not-allowed" disabled>View analytics (coming soon)</Button>
          <Button className="bg-primary text-white hover:bg-primary/90" onClick={resetCampaign}>Start new campaign</Button>
        </div>
      </motion.div>
    );
  }
    ...
    <div className="h-full flex flex-col md:flex-row gap-6 p-6">
      <div className="flex-1 bg-card border border-border rounded-xl p-6 flex flex-col overflow-y-auto">
        ...
        <div className="max-w-md mx-auto w-full bg-background border border-border rounded-xl overflow-hidden shadow-2xl">
          ...
          {anchor && <div className="w-full aspect-square bg-secondary"><img src={anchor.url} alt="Final" className="w-full h-full object-cover" /></div>}
          <div className="p-4 flex gap-4 border-t border-border">
            {[1, 2, 3].map(i => <div key={i} className="w-16 h-4 bg-secondary rounded" />)}
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-[400px] flex flex-col gap-4">
        <div className="bg-card border border-border rounded-xl p-4 flex flex-col flex-1 overflow-y-auto space-y-6">
              ...
              {PLATFORMS.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => toggle(p.id)}
                    className={`flex items-center gap-2 p-3 rounded-lg border text-sm transition-colors ${isSelected ? 'border-primary bg-primary/10 text-primary' : `border-border bg-background text-muted-foreground ${p.color}`}`}>
          ...
          <section>
            ...
            <Textarea value={caption} onChange={e => setCaption(e.target.value)} className="min-h-[150px] bg-background border-border text-sm resize-none" />
            <div className="flex justify-between items-center mt-2">
              <span className="text-xs text-muted-foreground">Auto-adjusted per platform</span>
              <span className={`text-xs ${caption.length > 2200 ? 'text-red-400' : 'text-muted-foreground'}`}>{caption.length} / 2200</span>
            </div>
          </section>
        ...
        <Button
          ...
          className="w-full h-14 bg-gradient-to-r from-primary to-[#9D84FD] text-white font-medium text-lg rounded-xl shadow-lg shadow-primary/20"
        >
