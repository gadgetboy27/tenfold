...
function InsightRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-3">
      <span className="text-xs text-muted-foreground uppercase tracking-wider w-28 shrink-0 pt-0.5 font-mono">
        {label}
      </span>
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span
              className={`inline-block text-[10px] font-medium uppercase tracking-wider px-1.5 py-0.5 rounded-full border mt-1 ${meta.bg} ${meta.color}`}
            >
              {angle.goal}
            </span>
          </div>
          ...
          >
            <div className="px-4 pb-4 space-y-3 border-t border-border/50 pt-3">
              ...
              <div>
                ...
                <p className="text-xs text-foreground/70 font-mono bg-secondary/50 rounded p-2 leading-relaxed">
                  {angle.imagePrompt}
                </p>
              </div>
              <div className="flex flex-wrap gap-1">
                {angle.platforms.map((p) => (
                  <span
                    key={p}
                    className="text-[10px] px-2 py-0.5 rounded-full bg-secondary border border-border text-muted-foreground capitalize"
                  >
                    {p}
                  </span>
                ))}
...
export default function CampaignBriefPanel() {
  const { campaignBrief, setCampaignBrief, setPendingBriefPrompt } =
    useAppStore();
  ...
  // Above the early return — hook order must not depend on the brief.
  const ent = useEntitlements();

  if (!campaignBrief) return null;

  const brief: CampaignBrief = campaignBrief;
  const selectedAngle = brief.campaignAngles.find(
    (a) => a.id === selectedAngleId,
  );
      ...
      <div className="max-w-3xl mx-auto px-6 pt-8">
          ...
          {showInsights && (
            ...
            >
              <div className="rounded-xl border border-border bg-card p-4 space-y-3">
                ...
                <InsightRow
                  label="Platforms"
                  value={brief.recommendedPlatforms.join(", ")}
                />
              </div>
            </motion.div>
          )}
        ...
        {brief.suggestedQuestions.length > 0 && (
          <div className="mb-6 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20">
            <p className="text-xs text-amber-400 uppercase tracking-wider font-mono mb-2">
      ...
      <div className="fixed bottom-0 left-40 right-0 z-30 p-4 pointer-events-none">
        <div className="max-w-3xl mx-auto pointer-events-auto">
          ...
          {ent && !ent.watermarkFree && (
            // Before the credits are spent, not after. Finding out your paid-for
            // asset carries our mark AFTER paying is the moment a free tier
            // reads as a rip-off; knowing up front makes it a plan you chose.
            <p className="text-[11px] text-muted-foreground mt-2 text-right">
              Free plan — posts and downloads carry a small{" "}
