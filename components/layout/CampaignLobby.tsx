            ...
            <div className="flex items-center gap-3 p-3 rounded-xl bg-destructive/5 border border-destructive/20 mb-5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate italic">
                  &ldquo;{campaign.prompt.slice(0, 60)}
                  {campaign.prompt.length > 60 ? "…" : ""}&rdquo;
                </p>
              </div>
            </div>
...
export default function CampaignLobby() {
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      ...
      if (campaignsRes.ok)
        setCampaigns((await campaignsRes.json()) as CampaignRow[]);
      if (socialsRes.ok) {
  ...
  const handleCancel = async (c: CampaignRow) => {
    setCancelling(c.id);
    try {
      const res = await api(`/api/campaigns/${c.id}/cancel`, {
        method: "POST",
        workspaceSlug,
      });
      if (res.ok) {
        ...
        setCampaigns((prev) =>
          prev.map((x) =>
            x.id === c.id ? { ...x, status: result.status } : x,
          ),
        );
        toast.success(
  ...
  const handleResume = async (c: CampaignRow) => {
    ...
    try {
      ...
      const expansionData = {
        ...(full.expansion_data ?? {}),
      } as CampaignResume["expansion_data"];
      ...
      toast.error(
        (err as Error).message ?? "Failed to open campaign — please try again",
      );
    } finally {
      setResuming(null);
    }
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              ...
              {campaigns.map((c, i) => {
                ...
                const statusColor =
                  STATUS_COLORS[c.status] ?? STATUS_COLORS.expanding;
                ...
                return (
                  ...
                  >
                    {/* Thumbnail */}
                    <div className="relative w-14 h-14 rounded-lg overflow-hidden bg-secondary border border-border shrink-0 flex items-center justify-center">
                      {c.thumbnailUrl ? (
                        <Image
                          src={c.thumbnailUrl}
                          alt=""
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate mt-0.5 italic">
                        &ldquo;{c.prompt.slice(0, 70)}
                        {c.prompt.length > 70 ? "…" : ""}&rdquo;
                      </p>
                      ...
                      {!isGenerating && (
                        <CampaignProgress currentStep={c.current_step} />
                      )}
                      {isGenerating && generatingSecs > 0 && (
                        <p className="text-[10px] text-amber-400 mt-1">
                          {isStuck
                            ? "⚠ Generation may be stuck"
                            : `Generating for ${generatingSecs}s…`}
                        </p>
                      )}
                    </div>
