...
| 11  | Low                   | **Legacy routes use `getSession` + admin client by hand** (≈75 of 129) rather than `withWorkspace`, so they get no rate limiting and rely on each author remembering the `workspace_id` filter. Spot-checks found no missing filter, but the guarantee is by inspection.                                                                                                                                                                                           | **Fixed** (same day, follow-up commits) — all 64 remaining routes migrated; `getSession` is now lint-refused under `app/api`. §5.1.                                                                                                                                                                                                                                                                                                            |
...
limit → uniform 401/429/500. The 75 hand-rolled routes were correct by
inspection, which means the next one might not have been. **Done, same day:**
every one of them now goes through the wrapper, and `eslint.config.mjs`
refuses `@/lib/auth/session` under `app/api/**/route.ts`. The tenant
guarantee no longer depends on people.

Two things the migration surfaced, worth knowing:

- The rate-limit bucket is keyed by IP and shared across all wrapped routes,
  so polled reads are exempted (`rateLimit: false`) — see `app/api/CLAUDE.md`.
  `diagnostics/health`, which fires five upstream calls per request, went the
  other way (10/min).
- Two routes had bugs the uniform error path fixed for free:
  `campaigns/[id]/anchor` answered 500 to an unauthenticated call, and
  `workspaces/me` returned `err.stack` to the client.

Also checked while here (prompted by the question "are any API keys in the
repo or reachable from the site?"): no `.env` file has ever been committed
(`.env.example` only, placeholders); no key-shaped string exists anywhere in
git history; no `"use client"` file reads a non-`NEXT_PUBLIC_` env var;
`/api/public-env` serves exactly the four public values; `diagnostics/health`
returns valid/invalid per service, never key material.
