...
export default function FloatingPromptBar() {
  ...
  const runGenerate = async (finalPrompt: string) => {
    ...
    try {
      ...
      if (!campRes.ok) {
        const e = await campRes.json().catch(() => ({})) as { error?: string; issues?: string[]; refinedPrompt?: string };
        if (e.issues?.length) {
          toast.error(e.issues.join(' — '));
          if (e.refinedPrompt) toast(`Try: "${e.refinedPrompt}"`, { icon: '💡' });
        } else {
      ...
      const poll = async (): Promise<void> => {
        ...
        if (campaign.status === 'ready') {
          ...
          setGeneratedAssets(readyAssets.map(a => ({
            id: a.id, url: a.url, prompt: a.prompt || finalPrompt,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Describe mode */}
        {mode === 'describe' && (
          <form onSubmit={handleDescribeSubmit} className="flex items-center gap-3 px-4 py-3">
            <input
              value={prompt}
          ...
          <div className="px-4 py-3 space-y-2">
            ...
            {analyzing && (
              <p className="text-[11px] text-[#555] px-1">
                Crawling website and asking Claude to build your marketing brief — usually 15–25 seconds…
              </p>
            )}
          </div>
        )}
