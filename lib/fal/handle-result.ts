...
//
// This used to live inside app/api/webhooks/fal/route.ts, which made the
// webhook the ONLY path that could turn a finished fal request into an asset.
// fal completes a Recraft logo concept in ~7–12s but its webhook delivery has
// been measured arriving 30–60s later, and once 75 minutes later, so a poller
// that finds a request COMPLETED on fal's queue (lib/logo/reclaim.ts) needs to
// save it exactly as the webhook would — same storage path, same metadata,
// same completion gate. One implementation, two callers.
 ...
 */
export function extractFalError(payload: {
  error?: unknown;
  payload?: unknown;
}): string {
  const detail = (
...
export async function handleSuccess(
  job: CreativeJob,
  payload: ResultData,
  direction: JobDirection | null = null,
