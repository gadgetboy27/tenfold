...
async function publishToFacebook(profile: SocialProfile, asset: Asset, caption: string): Promise<string> {
  if (!profile.platform_page_id) throw new Error('Facebook Page ID not found');
  if (asset.type === 'video') {
    return publishVideoToFacebook(profile.platform_page_id, profile.access_token, asset.url, caption);
  }
  return publishPhotoToFacebook(profile.platform_page_id, profile.access_token, asset.url, caption);
}

async function publishToInstagram(profile: SocialProfile, asset: Asset, caption: string): Promise<string> {
  const igUserId = profile.platform_account_id;
  if (!igUserId) throw new Error('Instagram account ID not found');
  if (asset.type === 'video') {
    return publishVideoToInstagram(igUserId, profile.access_token, asset.url, caption);
  }
  return publishPhotoToInstagram(igUserId, profile.access_token, asset.url, caption);
}
  ...
  try {
    ...
    if (!profiles || profiles.length === 0) {
      return NextResponse.json(
        { error: 'No connected accounts for the selected platforms. Go to Settings → Social to connect.' },
        { status: 422 },
      );
    }
    ...
    for (const profile of profiles as SocialProfile[]) {
      try {
        let postId: string;
        if (profile.platform === 'facebook')  postId = await publishToFacebook(profile, asset, fullCaption);
        else if (profile.platform === 'instagram') postId = await publishToInstagram(profile, asset, fullCaption);
        else { errors[profile.platform] = 'Platform publishing not yet supported'; continue; }
        platformResults[profile.platform] = postId;
      } catch (err) {
        errors[profile.platform] = err instanceof Error ? err.message : 'Unknown error';
      }
    }

    if (Object.keys(platformResults).length === 0) {
      return NextResponse.json({ error: 'All platforms failed to publish', errors }, { status: 500 });
    }
    ...
    return NextResponse.json({ record, platformResults, errors: Object.keys(errors).length ? errors : undefined }, { status: 201 });
  } catch (err) {
