...
export default function JobStatusIndicator() {
  ...
  const activeExpansions = Object.entries(expansions).filter(([, exp]) => exp?.status === 'pending');
          ...
          {isGenerating && (
            <div className="space-y-2">
              ...
              <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden relative">
                <motion.div className="h-full bg-primary" initial={{ width: '0%' }} animate={{ width: '100%' }} transition={{ duration: 3, ease: 'linear' }} />
              </div>
            </div>
          )}
          {activeExpansions.map(([type]) => (
