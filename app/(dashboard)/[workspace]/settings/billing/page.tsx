...
import {
  CreditCard, Zap, CheckCircle2, Loader2, TrendingUp,
  ArrowUpRight, ArrowDownRight, RotateCcw, Crown, Sparkles,
} from 'lucide-react';
...
export default function BillingPage() {
  ...
  useEffect(() => {
    if (success) toast.success('Payment successful — credits added to your account!');
  }, [success]);

  const handlePurchase = async (priceId: string | null, label: string) => {
    if (!priceId) { toast.error('This plan is not yet configured — contact support'); return; }
  ...
  return (
    <div className="max-w-4xl space-y-10">
      ...
      {/* Header */}
      <div>
        <h1 className="font-serif text-2xl font-bold text-foreground mb-1">Billing</h1>
        <p className="text-sm text-muted-foreground">Manage your plan and credit balance.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-2xl p-6 flex items-center gap-5">
          ...
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-mono mb-0.5">Credit Balance</p>
            <p className="text-3xl font-bold text-foreground tabular-nums">{data.balance.toLocaleString()}</p>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 flex items-center gap-5">
          <div className={cn(
            ...
            <Crown className={cn('w-6 h-6', tier === 'payg' ? 'text-muted-foreground' : 'text-amber-400')} />
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-mono mb-0.5">Current Plan</p>
            <p className="text-xl font-bold text-foreground">{tierLabel}</p>
            {periodEnd && (
              <p className="text-xs text-muted-foreground mt-0.5">Renews {fmt(periodEnd)}</p>
            )}
            {tier === 'payg' && (
              <p className="text-xs text-muted-foreground mt-0.5">No monthly credits</p>
            )}
      ...
      {/* Subscription plans */}
      <section>
        <h2 className="text-base font-semibold text-foreground mb-1">Monthly Plans</h2>
        <p className="text-sm text-muted-foreground mb-5">Credits reset each billing period. All prices in NZD.</p>
        <div className="grid grid-cols-3 gap-4">
          {data.plans.map(plan => {
            const isCurrent = tier === plan.id;
            const isLoading = purchasing === plan.priceId;
            return (
              <div
                key={plan.id}
                className={cn(
                  'relative rounded-2xl border p-6 flex flex-col gap-4',
                  plan.popular ? 'border-primary/40 bg-primary/5' : 'border-border bg-card',
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-1 mt-1">
                    <span className="text-2xl font-bold text-foreground">${plan.priceNzd}</span>
                ...
                <Button
                  ...
                  className={cn('w-full gap-2', plan.popular && !isCurrent && 'bg-primary hover:bg-primary/90 text-white')}
                  size="sm"
                >
                  {isLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                  {isCurrent ? 'Current plan' : `Subscribe — $${plan.priceNzd}/mo`}
                </Button>
              </div>
            );
          })}
      ...
      {/* Credit packs */}
      <section>
        <h2 className="text-base font-semibold text-foreground mb-1">Credit Packs</h2>
        <p className="text-sm text-muted-foreground mb-5">One-off top-ups. Credits never expire.</p>
        <div className="grid grid-cols-3 gap-4">
          {data.packs.map(pack => {
            const isLoading = purchasing === pack.priceId;
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    <Zap className={cn('w-5 h-5', pack.popular ? 'text-primary' : 'text-muted-foreground')} />
                  </div>
                  <div>
                ...
                <Button
                  ...
                  className={cn('w-full gap-2', pack.popular && 'bg-primary hover:bg-primary/90 text-white')}
                  size="sm"
                >
                  {isLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
      ...
      <section>
        ...
        <p className="text-sm text-muted-foreground mb-5">Last 30 transactions.</p>
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary/50">
                <tr>
                  ...
                  <th className="text-right px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Credits</th>
              ...
              <tbody>
                {data.transactions.map((tx, i) => (
                  <tr
                    key={tx.id}
                    className={cn('border-t border-border', i % 2 === 0 ? 'bg-card' : 'bg-secondary/20')}
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {TX_ICONS[tx.type] ?? <CreditCard className="w-3.5 h-3.5 text-muted-foreground" />}
