...
export async function GET(
  ...
  try {
    ...
    const stuckJobs = jobList.filter(
      (j: {
        status: string;
    ...
    if (stuckJobs.length > 0) {
      ...
      if (refreshedAssets) {
        const allDone2 =
          jobList.length > 0 &&
          jobList.every((j: { status: string }) => j.status === "completed");
        const anyFailed2 =
          jobList.length > 0 &&
          jobList.some((j: { status: string }) => j.status === "failed");
        const status2 = allDone2
          ? "ready"
          : anyFailed2
            ? "failed"
...
export async function PATCH(
  ...
  try {
    ...
    if (raw.current_step !== undefined)
      update.current_step = Math.min(6, Math.max(1, Number(raw.current_step)));
    if (raw.expansion_data !== undefined)
